...
kubectl -n soc-rapid-block create secret docker-registry ghcr-pull \
...
# 2.5 Kiểm tra
kubectl -n soc-rapid-block get pods,svc,pvc
kubectl -n soc-rapid-block logs deploy/soc-rapid-block-app -f
```
...
1. **Workspace ▸ Project**: tạo project (namespace) tên `soc-rapid-block` trong workspace `soc`.
...
6. **Network ▸ Services**: kiểm tra `soc-rapid-block` có port `30880/TCP` trên tất cả node.
...
# Rollback
kubectl -n soc-rapid-block rollout undo deploy/soc-rapid-block-app
```
...
- Kiểm nhanh từ pod: `kubectl -n soc-rapid-block exec deploy/soc-rapid-block-app -- curl -kv https://<firewall>:4444/webconsole/APIController`.
...
```bash
kubectl -n soc-rapid-block exec soc-rapid-block-db-0 -- \
...
kubectl delete pvc -n soc-rapid-block -l app.kubernetes.io/name=soc-rapid-block-db  # xoá dữ liệu DB
```
